...
class rtcBASE(threading.Thread):
    ...
    def isEventStored(self,mac="",user="",ip="",hostname="",observable=""):
        ...
        if rsp["rtcResult"] == "OK":
            ...
            self.logger.log_debug(0,"rtc db.getEvents failed {}".format(rsp["info"]))
        return False
    ...
    def getISEcontext(self,ip):
        mac = ""
        username = ""
        ## note with 204 (empty) ISE returns no json causing excaption in cats, TODO how to fix permanently
        ## ore logical to fix in cats
        try:
...
'''
                    ...
                    if not self.isEventStored(mac=thismac,user=thisusername,ip=thisIP,hostname=thishostname,observable=observable):
                        ...
                        self.logger.log_debug(2,"New event found - MAC {},IP {},user {}, hostname {},details {}".format(thismac,thisIP,thisusername,thishostname,json.dumps(eventDetails)))
                        ...
                        if thishostname and not hostnamePunished:
                            ...
                            try:
                                if int(hostnames["items"][0]["penalty"]) >= int(self.threshold):
                                    if "AMP_connector_guid" in eventDetails:
                                        self.amp.startHostIsolation(eventDetails["AMP_connector_guid"])
                            except Exception as e:
                                self.logger.log_debug(0,"should not happen hostnames is {}".format(json.dumps(hostnames)))
                                raise
                        else:
...
def main(argv):
    ...
    try:
        ...
        logger.log_debug(1,"starting rtcMain")
        ...
        logger.log_debug(1,"exception parsing options:"+logger.exception_info(err))        
        print_help()
        sys.exit(2)

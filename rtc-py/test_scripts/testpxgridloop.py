...
def main(argv):
    ...
    try:
        ...
        # get all pxGrid sessions
        rsp = i.getSessions()
        print(json.dumps(rsp,indent=4,sort_keys=True))

        for session in rsp["sessions"]:
            # do lookup of each session
            mac = session["macAddress"]
            print("getting MAC {}".format(mac))
            macrsp = i.getSessions(mac=mac)
            print(json.dumps(macrsp,indent=4,sort_keys=True))
